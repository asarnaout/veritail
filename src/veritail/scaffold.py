...
DEFAULT_SUGGEST_ADAPTER_FILENAME = "suggest_adapter.py"
...
SUGGEST_ADAPTER_TEMPLATE = """\
\"\"\"Generated by `veritail init --autocomplete`.

This adapter calls a suggest/autocomplete endpoint. Update the URL, auth,
and response parsing to match your autocomplete API.
\"\"\"
...
def scaffold_project(
    ...
    When autocomplete=True, also generates suggest_adapter.py and prefixes.csv.
    """
    ...
    if autocomplete:
        all_paths.append(target_dir / DEFAULT_SUGGEST_ADAPTER_FILENAME)
    ...
    if autocomplete:
        suggest_path = target_dir / DEFAULT_SUGGEST_ADAPTER_FILENAME
        prefixes_path = target_dir / DEFAULT_PREFIXES_FILENAME
        suggest_path.write_text(SUGGEST_ADAPTER_TEMPLATE, encoding="utf-8")
        prefixes_path.write_text(PREFIXES_TEMPLATE, encoding="utf-8")
        return adapter_path, queries_path, suggest_path, prefixes_path
...
__all__ = [
    ...
    "DEFAULT_SUGGEST_ADAPTER_FILENAME",
